...
document.querySelector("#addBook").addEventListener("submit", (e) => {
  ...
  const isWeNeedToEditBook = document.querySelector("#exisitingValue").value;
...
const formField = document.querySelectorAll(".Rfield");

formField.forEach((e) => {
  e.addEventListener("input", () => checkFormField(e));
  checkFormField(e);
});
...
function displayBooks() {
  ...
  myLibrary.forEach((e, index) => {
    ...
    const readingExperienceContainer = templateClone.querySelector(
      ".reading-experience",
    );
